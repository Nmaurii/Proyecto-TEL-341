...
def inicializar_nodos(cantidad,x,y):
    return [[random.randint(0,x),random.randint(0,y),0] for _ in range(cantidad)]

def agregar_nodo(nodos,x,y):

    nodos.append([random.randint(0,x),random.randint(0,y),0])
...
def generar_conexiones(nodos, enlaces, tolerancia,maximo):
    ...
    for i in range(len(nodos)):
        for j in range(i + 1, len(nodos)):
            ...
            if dist <= tolerancia and (nodos[i][2] <= maximo and nodos[j][2] <= maximo):
                peso = int(dist)
...
def crear_grafo(nodos, enlaces):
    ...
    G = nx.Graph()
    for i, (x,y,z) in enumerate(nodos):
